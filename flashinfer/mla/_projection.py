...
_GEMM_TACTIC = SplitKTactic(mma_m=64, mma_n=96, split_k=4, ab_stages=3)
...
def _check_inputs(
    ...
    _check_tensor(
        qkv_a_weight,
        "qkv_a_weight",
        shape=(_OUT_FEATURES, _IN_FEATURES),
        device=device,
        dtypes=(torch.bfloat16,),
    )
    _check_tensor(
...
def fused_qkv_a_proj_norm_rope(
    ...
    qkv_a_weight : torch.Tensor
        Contiguous BF16 linear weight with shape ``(2112, 7168)``.
    q_norm_weight : torch.Tensor
...
__all__ = ("fused_qkv_a_proj_norm_rope",)
